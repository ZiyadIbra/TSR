...
def preprocess_image(image):
    ...
    # Normalize pixel values (assuming the image is grayscale)
...
def classify(file_path):
    ...
    label.configure(text="Recognized Sign: " + sign)

def show_classify_button(file_path):
    classify_b = Button(top, text="Classify Image", command=lambda: classify(file_path), padx=12, pady=5)
    classify_b.configure(background='#975641', foreground='white', font=('arial', 10, 'bold'))
...
upload.configure(background='#46759D', foreground='white', font=('arial', 10, 'bold'))
